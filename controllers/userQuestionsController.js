...
const addOne = async (req, res) => {
  try {
    const {structured_question, leetcode_id, question_name, question_slug, question_difficulty, unstructured_question_body } = req.body;
    let newQuestion;
    console.log(structured_question);
    if (structured_question) {
      if (!leetcode_id || !question_name || !question_slug || !question_difficulty) {
      ...
      if (!unstructured_question_body || !question_name) {
        return res.status(400).send("All fields are required");
      }
      let newQuestionId = await knex("user_questions").insert({
          question_name,
...
try {
    ...
    const newSubmissionId = await knex("submissions").insert({user_question_id, submission_analyses});
    const newSubmission = await knex.select("*").from("submissions").where({id: newSubmissionId[0]});
...
const getOne = async (req, res) => {
    try {
        ...
        const submissions = await knex.select("*").from("submissions").where({user_question_id: id})
